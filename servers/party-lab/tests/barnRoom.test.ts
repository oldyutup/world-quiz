...
function skip(room: PartyRoom) {
  ...
  if (room.game instanceof LayerRoundSimulation || room.game instanceof ColorRoundSimulation) {
    // Countdown/results: to their last ticks. Play: everyone but the lowest slot falls out (last alive wins).
    const round = room.game.round,
      ticks = room.game instanceof LayerRoundSimulation ? LAYER_TICKS : COLOR_TICKS;
...
test("mode selector: the creator is host; only the host changes it; everyone sees it; a change clears every Ready", async () => {
  ...
  assert.ok(["rooftop_brawl", "barn_shootout", "layer_chaos", "color_chaos"].includes(b.room.state.mode), "Mixed shows a real mode");
...
test("Mixed: all four modes once per shuffled cycle, never twice in a row; each switch rebuilds and disposes the simulation; 20 switches do not leak", { timeout: 60000 }, async () => {
  const a = await create("Alice"),
    b = await join(a.room.roomId, "Bobby");
  const room = local(a);
  a.room.send("mode", "mixed");
  ...
  for (let c = 0; c + 4 <= played.length; c += 4)
    assert.deepEqual([...played.slice(c, c + 4)].sort(), ["barn_shootout", "color_chaos", "layer_chaos", "rooftop_brawl"], `cycle ${c / 4} has every mode once (${played.join(",")})`);
  ...
  assert.throws(() => (old as BarnRoundSimulation | OnlineRoundSimulation | LayerRoundSimulation | ColorRoundSimulation).physics.world.step(), "a disposed world cannot be stepped");
  await close(a, b);
});
