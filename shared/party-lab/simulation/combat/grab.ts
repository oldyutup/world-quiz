...
import {
  ...
  type Hand,
  type PartName,
} from "../ragdoll/config.js";
import { handPoint } from "../ragdoll/controller.js";
import {
  add,
  sub,
  mul,
...
import { resistance, type KnockoutState } from "./knockout.js";
...
export class HandGrips {
  ...
  reach(
    ...
    for (const other of this.physics.players) {
      ...
      for (const part of PARTS) {
        ...
        if (finite(point) && this.physics.clearPath(palm, point)) {
          ...
          if (
            part !== preferOtherPart &&
            (!alternate || distance < alternate.distance)
          )
            alternate = reach;
        }
      }
    ...
    return preferOtherPart &&
      alternate &&
      alternate.distance <= COMBAT.grip.range
      ? alternate
      : nearest;
  }
  acquire(
    ...
    this.stats.grabs++;
    this.feedback({
      name: this.count(owner, target) === 2 ? "secondGrab" : "grab",
      actor: owner,
      target,
      x: point.x,
    });
  ...
  release(owner: PlayerId, hand: Hand, reason = "release") {
    ...
    if (reason === "escape" || reason === "overload" || reason === "distance")
      this.feedback({
        ...
        actor: owner,
        target: grip.target,
        x: this.physics.players[owner].body.translation().x,
      });
      ...
      this.feedback({
        name: speed >= 2.5 ? "throw" : "release",
        actor: owner,
        target: grip.target,
        x: body.translation().x,
        intensity: clamp(speed / 10),
      });
    }
    ...
    for (const { id: owner } of PLAYERS)
      for (const hand of HANDS) {
        ...
        if (
          !(
            inputs[owner]?.grab ??
            (hand === 0 ? inputs[owner]?.left : inputs[owner]?.right)
          )
        ) {
