...
export class LayerRound {
  ...
  get seconds() {
    ...
    const total = this.phase === "countdown" ? LAYER_TICKS.countdown : LAYER_TICKS.results;
  ...
  private advance(eliminations: readonly PlayerId[]): LayerRoundEvent {
    if (this.phase === "countdown") {
      if (++this.tick < LAYER_TICKS.countdown) return null;
      this.phase = "playing";
      this.tick = 0;
      return "started";
    }
    if (this.phase === "results") {
      if (++this.tick < LAYER_TICKS.results) return null;
    ...
    if (this.tick >= LAYER_TICKS.cap - 1) return this.finish(null, "timeout");
    this.tick++;
    return null;
  }
}
