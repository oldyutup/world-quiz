-- ============================================================================
-- Wheel Group — RLS hardening · M1 (saf additive altyapı)
-- ============================================================================
-- AMAÇ
-- ----
-- Çark Çok Oyunculu (Wheel Group, 3–10 kişi) tablolarını ileride RPC-only
-- yazma modeline geçirmek için gereken altyapıyı oluşturur. Bu migration
-- HİÇBİR mevcut davranışı değiştirmez — yalnız yeni nesneler ekler. Mevcut
-- "_anon" RLS politikaları yerinde kalır, frontend dokunulmaz, oyun
-- mid-flight kırılmaz.
--
-- DOKUNULMAYAN ŞEYLER
-- -------------------
...
--   • wheel_duel_rooms / wheel_duel_players / wheel_duel_player_claims
--   • flag_duel_queue / wheel_duel_queue
--   • conquest_*
--   • profiles, xp_events
--   • award_wheel_group_xp_event RPC (20260518130000)
--   • wheel_group_check_capacity trigger (20260519120000)
--   • Mevcut wheel_group_rooms / wheel_group_players RLS politikaları
--     (wheel_group_rooms_select_all, wheel_group_rooms_insert_anon,
--      wheel_group_rooms_update_anon, wheel_group_rooms_delete_anon,
--      wheel_group_players_*) — bilinçli olarak yerinde bırakılıyor;
--      M3'te değiştirilecek.
--   • Mevcut wheel_group_rooms / wheel_group_players SATIRLARI (backfill yok)
--   • supabase_realtime publication üyelikleri (wheel_group_rooms,
--     wheel_group_players) — claim tablosu KASTEN dahil edilmiyor.
--
-- YENİ NESNELER
-- -------------
--   • wheel_group_players tablosuna iki yeni kolon:
--       profile_id  uuid null   — logged-in oyuncunun auth.users(id)'si
--       guest_id    text null   — misafir oyuncunun client-üretimli id'si
--   • public.wheel_group_player_claims
--       (player_id PK, claim_token uuid, created_at) — INSERT-only, realtime
--       DIŞI, SELECT grant yok.
--   • public.wheel_group_authorize_player(uuid, uuid) — yetki helper
--   • public.wheel_group_authorize_host  (uuid, uuid, uuid) — host yetki helper
--
-- IDEMPOTENT
-- ----------
...
-- Wheel Duel hardening (M1/M2/M3) pattern'inin birebir Wheel Group karşılığı.
-- ============================================================================


-- ────────────────────────────────────────────────────────────────────────────
-- 1) wheel_group_players — kimlik kolonları
-- ----------------------------------------------------------------------------
...
alter table public.wheel_group_players
...
-- ────────────────────────────────────────────────────────────────────────────
-- 2) wheel_group_player_claims — özel claim-token deposu
-- ----------------------------------------------------------------------------
-- Token'lar wheel_group_players ile aynı tabloda DURMAZ; eklenirse REPLICA
...
create table if not exists public.wheel_group_player_claims (
...
alter table public.wheel_group_player_claims enable row level security;
...
drop policy if exists "wheel_group_player_claims_insert"
  on public.wheel_group_player_claims;

create policy "wheel_group_player_claims_insert"
  on public.wheel_group_player_claims
  for insert
...
revoke select, update, delete on public.wheel_group_player_claims from anon, authenticated;
grant  insert                   on public.wheel_group_player_claims to anon, authenticated;

-- NOT: alter publication supabase_realtime ADD TABLE wheel_group_player_claims
...
-- ----------------------------------------------------------------------------
-- Conquest / Wheel Duel pattern'inin birebir Wheel Group karşılığı. M2 RPC'leri
-- bu helper'ları giriş noktası olarak kullanacak; M1 sırasında çağıran yok ama
...
revoke all     on function public.wheel_group_authorize_player(uuid, uuid) from public;
grant  execute on function public.wheel_group_authorize_player(uuid, uuid) to anon, authenticated;


-- (b) Verilen oda için: caller, oda host'unun ta kendisi mi?
--     host_player_id eşleşmesi + authorize_player ZORUNLU.
create or replace function public.wheel_group_authorize_host(
  p_room_id     uuid,
  p_player_id   uuid,
  p_claim_token uuid
) returns boolean
  ...
  select exists (
    select 1
     ...
     where r.id = p_room_id
       ...
       and public.wheel_group_authorize_player(p_player_id, p_claim_token)
  );
$$;

revoke all     on function public.wheel_group_authorize_host(uuid, uuid, uuid) from public;
grant  execute on function public.wheel_group_authorize_host(uuid, uuid, uuid) to anon, authenticated;
...
--      and table_name   = 'wheel_group_players'
--      and column_name in ('profile_id', 'guest_id');
--
--   -- Yeni tablo:
--   select table_name from information_schema.tables
--    where table_schema = 'public' and table_name = 'wheel_group_player_claims';
--
--   -- Policy yalnız INSERT olmalı:
--   select policyname, cmd, roles
--     from pg_policies
--    where schemaname = 'public' and tablename = 'wheel_group_player_claims';
--
--   -- Grant durumu: anon/authenticated sadece INSERT'e sahip olmalı.
--   select grantee, privilege_type
--     from information_schema.role_table_grants
--    where table_schema = 'public'
--      and table_name   = 'wheel_group_player_claims'
...
--   -- Beklenen liste: yalnızca wheel_group_rooms ve wheel_group_players.
--
--   -- Fonksiyonlar (security definer flag'i):
--   select proname, prosecdef
--     from pg_proc
--    where pronamespace = 'public'::regnamespace
--      and proname in ('wheel_group_authorize_player', 'wheel_group_authorize_host');
--
