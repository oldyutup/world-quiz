...
import { IDLE_INPUT, type PlaygroundPhysics } from "./physics";
import { PLAYERS } from "./players";
import { HANDS, HAND_PARTS, PARTS, type Hand } from "./ragdoll/config";
import { normalDrive } from "./ragdoll/controller";
import {
  add,
  sub,
  mul,
...
import type { RoundPhase } from "./roundLogic";
...
export class CombatSimulation {
  ...
  step(inputs: readonly MovementInput[], dt: number, phase: RoundPhase) {
    ...
    for (const p of this.players) {
      ...
      const firstReachHand = input.grab && !this.grips.count(p.id)
        ? HANDS.filter(hand => p.punches[hand].age < 0 && !this.grips.hands[p.id][hand])
            .map(hand => ({ hand, reach: this.grips.reach(p.id, hand) }))
            .sort((a, b) => (a.reach?.distance ?? Infinity) - (b.reach?.distance ?? Infinity))[0]?.hand
        : undefined;
      for (const hand of HANDS) {
        const down = input.grab ?? (hand === 0 ? !!input.left : !!input.right),
          request = input.punch
            ? hand === requestedHand
            : hand === 0 ? input.punchLeft : input.punchRight;
        ...
        const reaching =
          down &&
          (input.grab !== undefined || p.heldFor[hand] >= COMBAT.holdThreshold) &&
          punch.age < 0;
