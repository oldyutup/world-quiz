...
export const ROUND = { countdown: 3, duration: 60, results: 3.5, finalFallGrace: 0.35 } as const;
...
export class RoundLogic {
  ...
  get seconds(): number {
    const duration = this.phase === "countdown" ? ROUND.countdown : this.phase === "playing" ? ROUND.duration : ROUND.results;
    return Math.max(0, Math.ceil(duration - this.elapsed - EPSILON));
  }

  snapshot(): RoundSnapshot {
    return { phase: this.phase, seconds: this.seconds, alive: [...this.alive], winner: this.winner, reason: this.reason };
  }

  tick(dt: number, eliminations: readonly PlayerId[] = []): RoundEvent {
    const oldPhase = this.phase;
    const oldSeconds = this.seconds;
    const oldAlive = this.alive.reduce((mask, alive, id) => mask | (Number(alive) << id), 0);
    const event = this.advance(dt, eliminations);
    const newAlive = this.alive.reduce((mask, alive, id) => mask | (Number(alive) << id), 0);
    if (oldPhase !== this.phase || oldSeconds !== this.seconds || oldAlive !== newAlive) this.revision++;
    return event;
  }

  private finish(winner: PlayerId | null, reason: RoundSnapshot["reason"]): RoundEvent {
    this.phase = "results";
    this.elapsed = 0;
    this.winner = winner;
    this.reason = reason;
  ...
  private advance(dt: number, eliminations: readonly PlayerId[]): RoundEvent {
    ...
    if (survivors.length === 1) {
      this.finalFallElapsed = this.finalFallElapsed === null ? 0 : this.finalFallElapsed + dt;
      if (this.finalFallElapsed + EPSILON >= ROUND.finalFallGrace) return this.finish(survivors[0].id, "survivor");
      return null; // Let an already-falling final bean finish falling, even at timeout.
    }
    if (this.elapsed + EPSILON >= ROUND.duration) return this.finish(null, "timeout");
    return null;
  }
}
